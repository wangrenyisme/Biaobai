		...
		<div style="width: 100%;">
			<div id="title">
				<div
					...
					<img src="<%=request.getContextPath()%>/headphoto/<%=DaoFactory.getUserDao().getByUserName(biaobai2.getUsername()).getHeadphoto()%>" width="45" height="45" />
				</div>
				<div style="height: 45px; float: left; padding: 10px;"><a href="<%=request.getContextPath()%>/personl.jsp?username=<%=biaobai2.getUsername()%>" style="color:#000;"><%=biaobai2.getUsername()%></a></div>
			</div>
			<hr style=" clear: both;"/>
			<div style="margin: 5px 5px 5px 5px; clear: both;">
				<b>想认识的人:</b>
				一个<%=biaobai2.getGender()%><br>
				<b>我的联系方式：</b>
				<%=biaobai2.getContact()%><br>
				<b>我想说:</b>
				<%=biaobai2.getContent()%><br>
			</div>
			<hr style=" clear: both;"/>
			...
			<div style="margin: 10px 10px 10px 10px;">
				...
				<form action="<%=request.getContextPath()%>/CommentForBai1Add"
					...
					<input type="hidden" name="type" value="<%=biaobai2.getType()%>" />
					<input type="hidden" name="messageId" value="<%=id%>" />
					<input type="hidden" name="toUId"
						value="<%=biaobai2.getUsername()%>" />
			...
			<div id="ComContent"
				...
				<ul>
					<%
						List<Reply> comsAndReply = DaoFactory.getReplyDao().list(id, biaobai2.getType());
					...
					<li><div style="clear: both; width:100%;" >
							...
							<div style="float: left;width:90%;">
								...
								<button id="button" style="float: right;"  class="btn btn-info" 
									onclick="addComment(this,'2','<%=biaobai2.getType()%>','<%=id%>','<%=reply.getId()%>','<%=reply.getFrom_uid()%>','<%=loginUser.getUsername()%>')">回复</button>
								...
								<div style="clear: both; float: none;">
									...
									%><div style="margin: 15px 0px 10px 10px; margin-left: 20px;">
										...
										<button style="float: right;"  class="btn btn-info" 
											onclick="addComment(this,'2','<%=biaobai2.getType()%>','<%=id%>','<%=reply.getId()%>','<%=reply2.getFrom_uid()%>','<%=loginUser.getUsername()%>')">回复</button>
									</div>
