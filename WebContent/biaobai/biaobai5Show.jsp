...
<script type="text/javascript">
	...
	function addComment(btn, kind_tmp, type_tmp, messageId_tmp, replyId_tmp,
		...
		div.parentNode.innerHTML += "<form action='../CommentForBai1Add' method='post' onsubmit='getValue()'>"
				+ "<input type='text' name='content' style='80%;height:23px;margin:0px 0px 0px 10px;'>"
		...
		<div style="width: 100%;">
			<div id="title">
				<div
					...
					<img src="<%=request.getContextPath()%>/headphoto/<%=DaoFactory.getUserDao().getByUserName(biaobai5.getUsername()).getHeadphoto()%>" width="45" height="45" />
				</div>
				<div style="height: 45px; float: left; padding: 10px;"><a href="<%=request.getContextPath()%>/personl.jsp?username=<%=biaobai5.getUsername()%>" style="color:#000;"><%=biaobai5.getUsername()%></a></div>
			</div>
			<hr style=" clear: both;"/>
			<div style="margin: 5px 5px 5px 5px; clear: both;">
				<b>表白对象：</b>
				<%=biaobai5.getBobject() %><br >
				<b>标题：</b>
				<%=biaobai5.getTitle() %><br >
				<b>表白内容：</b>
				<%=biaobai5.getContent() %><br >
			</div>
			<hr style=" clear: both;"/>
			...
			<div style="margin: 10px 10px 10px 10px;">
				...
				<form action="<%=request.getContextPath()%>/CommentForBai1Add"
					...
					<input type="hidden" name="type" value="<%=biaobai5.getType()%>" />
					<input type="hidden" name="messageId" value="<%=id%>" />
					<input type="hidden" name="toUId"
						value="<%=biaobai5.getUsername()%>" />
			...
			<div id="ComContent"
				...
				<ul>
					<%
						List<Reply> comsAndReply = DaoFactory.getReplyDao().list(id, biaobai5.getType());
					...
					<li><div style="clear: both; width:100%;" >
							...
							<div style="float: left;width:90%;">
								...
								<button id="button" style="float: right;"  class="btn btn-info" 
									onclick="addComment(this,'2','<%=biaobai5.getType()%>','<%=id%>','<%=reply.getId()%>','<%=reply.getFrom_uid()%>','<%=loginUser.getUsername()%>')">回复</button>
								...
								<div style="clear: both; float: none;">
									...
									%><div style="margin: 15px 0px 10px 10px; margin-left: 20px;">
										...
										<button style="float: right;"  class="btn btn-info" 
											onclick="addComment(this,'2','<%=biaobai5.getType()%>','<%=id%>','<%=reply.getId()%>','<%=reply2.getFrom_uid()%>','<%=loginUser.getUsername()%>')">回复</button>
									</div>
