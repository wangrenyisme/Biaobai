		...
		<div style="width: 100%;">
			<div id="title">
				<div
					...
					<img src="<%=request.getContextPath()%>/headphoto/<%=DaoFactory.getUserDao().getByUserName(biaobai1.getUsername()).getHeadphoto()%>" width="45" height="45" />
				</div>
				<div style="height: 45px; float: left; padding: 10px;"><a href="<%=request.getContextPath()%>/personl.jsp?username=<%=biaobai1.getUsername()%>" style="color:#000;"><%=biaobai1.getUsername()%></a></div>
			</div>
			<hr style=" clear: both;"/>
			<div style="margin: 5px 5px 5px 5px; clear: both;">
				<b>我要表白:</b>
				<%=biaobai1.getDepartment()%>的<a href="<%=request.getContextPath()%>/personl.jsp?username=<%=biaobai1.getLovename()%>"><font style="color:red;font-size:30px;"><b><%=biaobai1.getLovename()%></b></font></a><%=biaobai1.getGender()%><br>
				<b>我想说:</b>
					...
					<li><div style="clear: both; width:100%;" >
							...
							<div style="float: left;width:90%;">
								...
								<button id="button" style="float: right;"  class="btn btn-info" 
									onclick="addComment(this,'2','<%=biaobai1.getType()%>','<%=id%>','<%=reply.getId()%>','<%=reply.getFrom_uid()%>','<%=loginUser.getUsername()%>')">回复</button>
								...
								<div style="clear: both; float: none;">
									...
									%><div style="margin: 15px 0px 10px 10px; margin-left: 20px;">
										...
										<button style="float: right;"  class="btn btn-info" 
											onclick="addComment(this,'2','<%=biaobai1.getType()%>','<%=id%>','<%=reply.getId()%>','<%=reply2.getFrom_uid()%>','<%=loginUser.getUsername()%>')">回复</button>
									</div>
