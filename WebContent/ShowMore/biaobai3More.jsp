		...
		<div class="layui-col-md6" style="width:80%;">
			<div class="layui-card" id="md6">
				<div id="title" class="layui-card-header">
					...
					<img src="<%=request.getContextPath()%>/headphoto/<%=DaoFactory.getUserDao().getByUserName(biaobai4.getUsername()).getHeadphoto()%>" width="45" height="45"/></div><!-- 头像 -->
					<!--  -->
					<div style="height:45px;padding:10px;"><%=biaobai4.getUsername()%></div>
				</div>
				<div class="layui-card-body">
					一个"<%=chara.length() > 20 ? chara.substring(0, 20) + "..." : chara%>"的<%=biaobai4.getGender()%>
										想找一个"<%=favor.length() > 20 ? favor.substring(0, 20) + "..." : favor%>"的<%=biaobai4.getGender().equals("小哥哥") ? "小姐姐" : "小哥哥"%>
										<br>
					<a
						href="<%=request.getContextPath()%>/biaobai/biaobai4Show.jsp?id=<%=biaobai4.getId()%>"
						style="float: right">
					...
					<div style="margin: 0px 5px 5px 5px;">
						<%
					if(biaobai4.getPhoto()!=null&&!"".equals(biaobai4.getPhoto().trim()))
					{
					...
					<div style="margin: 5px 5px 5px 5px; float: left;">
						<img src="<%=request.getContextPath()%>/fileUpload/<%=dir1+"/" + dir2 + "/"%><%=biaobai4.getPhoto()%>" width="100" height="100" />
					</div>
					<%
					}
						%>
